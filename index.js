...
app.get("/updates/:lines", async (req, res) => {
  const filterLines = req.params.lines
    ? req.params.lines.toUpperCase().split("")
    : null;
    ...
    feeds.forEach((feed) => {
      feed.entity.forEach((entity) => {
        ...
        if (
          ...
          (!filterLines || filterLines.includes(routeId))
        ) {
          entity.tripUpdate.stopTimeUpdate.forEach((stopTimeUpdate) => {
            console.log(routeId);
            if (stopTimeUpdate.stopId) {
              const baseStopId = stopTimeUpdate.stopId.replace(/[NS]$/, "");
              const stopName = stationsData[baseStopId];
              if (stopName) {
                updates.push({
                  entityId: entity.id,
                  routeId,
        ...
        entity.tripUpdate.stopTimeUpdate.forEach((stopTimeUpdate) => {
          ...
          matchingStations.forEach((station) => {
            ...
            if (parentStation == null) {
              parentStation =
                stopsData[routeId + entity.tripUpdate.trip.directionId];
            }
            updates.push({
              routeId,
...
app.listen(port, () => {
  console.log(`🚇 MTA Real-Time API running at http://localhost:${port}`);
});
